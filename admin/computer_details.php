...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($check->num_rows > 0) {
        // Update status
        $update = $conn->prepare("UPDATE pc_status SET status = ?, note = ?, updated_at = NOW() WHERE lab_id = ? AND pc_number = ?");
        $update->bind_param("ssii", $status, $note, $lab_id, $pc_number);
        $update->execute();
    } else {
        // Insert baru
        $insert = $conn->prepare("INSERT INTO pc_status (lab_id, pc_number, status, note) VALUES (?, ?, ?, ?)");
        $insert->bind_param("iiss", $lab_id, $pc_number, $status, $note);
        $insert->execute();
    }

    echo "Status berhasil disimpan";
    exit; // penting agar tidak render HTML kalau ini request AJAX
}
?>
...
<body>
  <header>
    ...
    <a href="data_lab.php" class="button"><i class="bi bi-arrow-left"></i> Kembali</a>
  </header>
    ...
    while ($lab = $result->fetch_assoc()) {
        ...
        for ($i = 1; $i <= $lab['total_pc']; $i++) {
            ...
            <div class='card'>
              ...
              <div class='status $statusClass' onclick=\"toggleStatus(this, {$lab['id']}, $i)\" title='$note'>$statusLabel</div>
            </div>";
        }
